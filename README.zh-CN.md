...
**错误与突发情况是机器人执行中无法回避的一部分，模型需要见过偏离计划的状态，也需要见过从这些状态恢复的过程。** 导航和操作中，机器人可能抓偏、意外松爪或行进偏航；长任务中的小偏差还可能逐步累积。成功示范展示了任务应该怎样完成，但仅有成功示范，无法提供足够的“出错以后怎么办”的例子。

我们的目标是构建**可控、可配对、可验证的反事实纠错数据集**：从一条成功轨迹出发，主动注入不同时间、类型、幅度、时长和次数的扰动，比较同一任务的不同后续过程。

1. **正常成功：** 不注入这次扰动，任务原本如何完成？
2. **错误对照：** 遭遇扰动后，继续原来剩余的动作、不根据状态调整，会发生什么？
3. **纠错恢复：** 从经过校验的同一个首次扰动后状态出发，根据当前状态调整动作，能否重新完成任务？

我们希望这些数据能用于研究模型如何识别偏差、选择纠正动作、重新规划，并在导航、操作及长程任务中应对突发情况。失败恢复也会保留，帮助暴露当前控制器的能力边界。模型需要学习的既包括顺利完成任务，也包括在执行偏离预期之后继续解决问题。

这里的“反事实”指在受控仿真条件下构造的替代执行过程。只有成功源、有效偏差、错误对照失败和恢复成功同时成立，才记为合格纠错。当前轨迹证明的是特定条件下的可恢复性；模型训练收益、对未见错误的泛化及真实机器人迁移仍需单独验证。[配对协议与证据要求](docs/correction-protocol.md)。

## 当前进展

**补上 5 个任务专家，均已有原生成功源。** YCB 抓取在新种子上 4/5；RoboCasa 新增任务仍属实验性。[实测结果与视频](https://pm1255.github.io/robot_stack/experts.html) · [启动示例](docs/new-experts.md)。
...
最新逐任务校准：实测 56 类任务，其中 28 类通过本轮小样本独立验证（MetaWorld 25、ManiSkill 3）。另有 23 类 MetaWorld 需完善场景指纹核验、3 类基线未达标、2 类尚无合格扰动；尚未完成全部 546 类任务。
