...
def save_episode_result(
    *,
    ...
    result = {
        ...
        "pickup_target_robot_xyz": [float(x) for x in args.pickup_target_robot],
        "place_target_robot_xyz": [float(x) for x in args.place_target_robot],
...
def execute_curobo_plan_on_a2d(
    *,
    ...
    for t, q in enumerate(positions):
        ...
        if t % save_stride == 0 or t == positions.shape[0] - 1:
            sample = {
                "index": int(t),
...
def main():
    ...
    try:
        ...
        for ep in range(args.num_pick_place):
            ...
            # Key change:
            # The helper above clamps to a known reachable matrix.
            # We then explicitly override xyz so pickup/place can be different.
            ...
            if args.execution_mode == "debug_object":
                ...
                pickup_executed = execute_curobo_plan_on_a2d(
                    ...
                    cup_path=args.cup_path if args.save_executed_trajectory else None,
            ...
            # Key change:
            # This makes place target different from pickup target.
            ...
            if args.execution_mode == "debug_object":
                ...
                place_executed = execute_curobo_plan_on_a2d(
                    ...
                    steps_per_position=args.steps_per_position,
                    speed_stride=args.speed_stride,
                    stage=stage if (args.attach_cup_during_place or args.save_executed_trajectory) else None,
                    cup_path=args.cup_path if (args.attach_cup_during_place or args.save_executed_trajectory) else None,
                    ee_path=args.ee_path if (args.attach_cup_during_place or args.save_executed_trajectory) else None,
