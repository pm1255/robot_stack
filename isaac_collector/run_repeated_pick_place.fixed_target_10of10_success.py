...
def validate_episode_basic(
    *,
    ...
    checks = {
        "initial_xyz": [float(x) for x in initial_xyz.tolist()],
        "final_xyz": [float(x) for x in final_xyz.tolist()],
        ...
        "final_z_ok": bool(final_xyz[2] >= min_final_z),
    }

    checks["success"] = bool(checks["moved_enough"] and checks["final_z_ok"])
    return checks
